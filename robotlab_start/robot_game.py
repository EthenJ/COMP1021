...
def makeDecisionForTask4():
    ...
    global find_the_wall_4
    if not upIsWall() and not find_the_wall_4:
        stateForTask4 = "Go up"
    else:
        if not find_the_wall_4:
            stateForTask4 = "Go left"
            find_the_wall_4 = True
        for i in range(0, 4):
            if stateForTask4 == forwards[i]:
                if not func_rights[i]():
                    stateForTask4 = rights[i]
                elif func_rights[i]():
                    if not func_forwards[i]():
                        stateForTask4 = forwards[i]
                    elif not func_lefts[i]():
                        stateForTask4 = lefts[i]
                    else:
                        stateForTask4 = backwards[i]
                break
            else:
                continue
    return execute(stateForTask4)
